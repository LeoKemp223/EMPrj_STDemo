...
void ReadID(void)
{
	...
	HAL_GPIO_WritePin(GPIO_CSS_GPIO_Port, GPIO_CSS_Pin, GPIO_PIN_RESET);	// 使能CS
	HAL_SPI_Transmit(&hspi1, &w25x_read_id, 1, 100);		// 读ID发送指令
	HAL_SPI_Receive(&hspi1, temp_ID, 5, 100);				// 读取ID
	HAL_GPIO_WritePin(GPIO_CSS_GPIO_Port, GPIO_CSS_Pin, GPIO_PIN_SET);	// 失能CS
